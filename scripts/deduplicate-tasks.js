...
const THRESHOLDS = {
  semantic: {
    min: 0.85,           // 85% - Minimum similarity to consider as duplicate
    highConfidence: 0.95 // 95% - Override frequency check (typos/exact matches)
  },
  frequency: {
...
async function analyzeDuplicates() {
  ...
  const results = {
    ...
    duplicate_pairs: duplicatePairs.map(p => ({
      taskA: { id: p.taskA.id, description: p.taskA.description },
      taskB: { id: p.taskB.id, description: p.taskB.description },
