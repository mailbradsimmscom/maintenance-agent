/**
 * TEST VERSION: Extract, Enrich, and Upload Tasks
 *
 * READS FROM: pinecone_search_results_test (test table)
 * WRITES TO: Pinecone MAINTENANCE_TASKS namespace (same as production)
 *
 * Flow:
 * 1. Read high-scoring chunks from pinecone_search_results_test (≥50%)
 ...
 *
 * Use this to test extraction on controlled dataset without affecting main table.
 */
...
async function extractEnrichAndUpload() {
  ...
  console.log('\n=== TEST: EXTRACT, ENRICH & UPLOAD TASKS ===\n');
  console.log('⚠️  TEST MODE - Reading from: pinecone_search_results_test\n');
  if (isDryRun) {
    console.log('🔍 DRY RUN MODE - No uploads will be performed\n');
  }
  console.log(`Threshold: ${SCORE_THRESHOLD}+ (${SCORE_THRESHOLD * 100}%)\n`);

  // Get high-scoring chunks FROM TEST TABLE
