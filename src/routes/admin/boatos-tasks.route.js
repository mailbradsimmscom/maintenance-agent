...
router.get('/due', async (req, res, next) => {
  ...
  try {
    logger.info('Fetching due BoatOS tasks');
    ...
    logger.error('Error fetching due tasks', { error: error.message });
    return next(error);
  }
});
...
router.get('/statistics', async (req, res, next) => {
  ...
  try {
    logger.info('Fetching BoatOS task statistics');
...
router.post('/:taskId/complete', async (req, res, next) => {
  ...
  try {
    ...
    logger.info('BoatOS task completed', { taskId, nextDue: updatedTask.next_due });
...
router.post('/:taskId/dismiss', async (req, res, next) => {
  ...
  try {
    ...
    logger.info('BoatOS task dismissed', { taskId });
...
router.post('/create/:assetUid', async (req, res, next) => {
  ...
  try {
    ...
    logger.info('Hours update task created', { assetUid, taskId: task.id });
    ...
    logger.error('Error creating task', { assetUid, error: error.message });
    return next(error);
  }
});
